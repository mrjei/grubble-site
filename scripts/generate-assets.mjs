/**
 * Generates raster brand assets from the locked Fork-G mark:
 *   public/og-image.png        1200×630  (Open Graph / Twitter card)
 *   public/apple-touch-icon.png 180×180  (iOS home screen)
 *   public/favicon.ico          32×32    (PNG wrapped in an ICO container)
 *
 * Run with `npm run gen:assets`. Uses sharp (bundled with Astro's image service).
 * favicon.svg is committed directly (vector) and used by modern browsers first.
 */
...
const squircleSvg = (size) =>
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="120 120 784 784" width="${size}" height="${size}"><path d="${SQUIRCLE}" fill="${PRIMARY}"/><path d="${FORK_G}" fill="${CREAM}"/></svg>`;

// Apple touch icon: full-bleed green ground + cream Fork-G (iOS rounds corners).
const appleSvg = (size) =>
  `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" width="${size}" height="${size}"><rect width="1024" height="1024" fill="${PRIMARY}"/><path d="${FORK_G}" fill="${CREAM}"/></svg>`;

// Open Graph card: cream canvas, squircle mark, wordmark + tagline.
const ogSvg = () => `
<svg xmlns="http://www.w3.org/2000/svg" width="1200" height="630" viewBox="0 0 1200 630">
  <rect width="1200" height="630" fill="${BG}"/>
  <g transform="translate(110 213) scale(0.262)">
    <path d="${SQUIRCLE}" fill="${PRIMARY}"/>
    <path d="${FORK_G}" fill="${CREAM}"/>
  </g>
  <text x="430" y="296" font-family="${FONT}" font-size="88" font-weight="800" letter-spacing="-2" fill="${INK}">Grubble Eats</text>
  <text x="433" y="364" font-family="${FONT}" font-size="40" font-weight="500" fill="${PRIMARY}">Decide what to eat, together.</text>
</svg>`;
...
async function run() {
  ...
  await sharp(Buffer.from(appleSvg(180)))
    .png()
    .toFile(join(PUBLIC, 'apple-touch-icon.png'));
  console.log('✓ apple-touch-icon.png (180×180)');

  const fav32 = await sharp(Buffer.from(squircleSvg(32))).png().toBuffer();
  writeFileSync(join(PUBLIC, 'favicon.ico'), buildIco(fav32));
